...
def test_build_human_review_packet_respects_target_count() -> None:
    records = [
        _record(
            f"exp-{index:02d}",
            input_text="永",
    ...
    packet = build_human_review_packet(records, target_count=32)

    assert packet["representative_count"] == 32
    assert len(packet["representatives"]) == 32
