...
このプロジェクトは最初に着手する。LLM エージェントが研究を主導する場合、評価 harness は単なる後処理ではなく、研究ループを成立させる実行基盤である。
...
4. LLM エージェントが評価結果と成果物を同じ ID で参照できれば、次実験の提案精度が上がる。
...
- LLM の主観評価だけに依存する採択判定。

## LLM Agent Workflow

LLM エージェントは次の順序で研究を進める。
