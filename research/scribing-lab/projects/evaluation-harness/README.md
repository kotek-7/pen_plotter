...
Tkinter 版より安定している。`Start Card` タブと `Export Start Card` ボタンで、開いた
packet の代表項目・注視点・script 分布をそのまま見せられる。`Preview Run` タブでは、
...
`human-feedback-review-bundle` を使うと、packet / Start Card / comparison sheet / review guide をまとめて出せる。
`human-feedback-review-ui` を使うと、その bundle をそのまま Qt UI で開ける。
