...
実装済みの MVP は、`baseline-neat` / `glyph-neat` / `kanji-neat` / `steady-neat` / `fast-casual` / `compact-casual` / `micro-casual` / `flow-casual` / `textured-casual` / `textured-steady` / `textured-tight` / `shaky-slow` の 12 種類の手動 profile を登録し、
