...
この評価基盤は、LLM エージェントが研究を進めるための最初の必須基盤である。生成モデルの実装より先に、実験設定、成果物、評価結果、失敗分類、次実験提案を記録できる状態にする。

## LLM Agent Requirements

LLM エージェントは、各実験で次を必ず残す。
...
評価基盤がこれらを保存できない段階では、motion model や neural variation の本格実装に進まない。
...
LLM エージェントが失敗を同じ語彙で分類できるよう、タグを固定する。
...
LLM エージェントが次の判断を行うため、各実験のレビュー入力は次の束にする。
...
LLM の役割は、主観的な採点だけではなく、次に検証すべき仮説と最小変更を提案することである。
