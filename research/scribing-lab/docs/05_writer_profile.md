...
初期実装では低次元の明示パラメータを優先する。短文評価で global parameter だけでは一貫性が足りない場合、`global`、`char_class`、`stroke_type`、`line_context` の階層 profile へ拡張する。

## few-shot 適応

初期段階では、少数サンプルから profile の傾向を推定できることを目指す。
...
- 同一 profile の短文で、字間、行方向、終筆癖が一貫しているか。
